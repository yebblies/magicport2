...
void scanElfObjModule(void delegate(char* name, int pickAny) pAddSymbol, void* base, size_t buflen, const(char)* module_name, Loc loc)
{
    ...
    if (buflen < EI_NIDENT + Elf32_Ehdr.sizeof)
    {
        reason = __LINE__;
    Lcorrupt:
    ...
    {
        Elf32_Ehdr* eh = cast(Elf32_Ehdr*)(buf + EI_NIDENT);
        if (eh.e_type != ET_REL)
        {
    ...
    {
        Elf64_Ehdr* eh = cast(Elf64_Ehdr*)(buf + EI_NIDENT);
        if (buflen < EI_NIDENT + Elf64_Ehdr.sizeof)
            goto Lcorrupt;
        if (eh.e_type != ET_REL)
        {
            error(loc, "ELF object module %s is not relocatable", module_name);
